...
int main()
{
    ...
    ll dp[k + 1][n]; // dp[j][i]  denotes you are starting at ith index and seeing 2^j from there -- or min of i to 2^j-1 elements
    ...
    while (q--)
    {
        ...
        for (int i = k; i >= 0; i--)
        {
            if ((1 << i) <= b - a + 1)  //checking most set bit and then reducing with it --
            {
                sum += dp[i][a];
                a += 1 << i;
            }
        }
        cout << sum << endl; //for sum you cant keep overlapping interval so you have to carefull about
    }
    return 0;
}