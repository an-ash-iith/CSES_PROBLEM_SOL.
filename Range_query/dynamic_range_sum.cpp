...
long long findsum(long long i, long long j, long long low, long long high, long long node, vector<long long> &tree)
{
    if (i <= low && high <= j)
        return tree[node];
    ...
    return findsum(i, j, low, mid, 2 * node + 1, tree) + findsum(i, j, mid + 1, high, 2 * node + 2, tree);
}
...
long long update(long long index, long long val, long long low, long long high, long long node, vector<long long> &tree)
{
    ...
    tree[node] = update(index, val, low, mid, 2 * node + 1, tree)+update(index, val, mid + 1, high, 2 * node + 2, tree);
    return tree[node];
}
...
int main()
{
    ...
    while (q--)
    {
        ...
        {
            b--;
            cout << findsum(a, b, 0, n - 1, 0, tree) << endl;
        }
    }
